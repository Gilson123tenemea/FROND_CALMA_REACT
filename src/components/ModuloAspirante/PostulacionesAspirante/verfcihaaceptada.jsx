...
const VerFichaAceptada = () => {
    ...
    useEffect(() => {
        const fetchFicha = async () => {
            try {
                setLoading(true);
                setError(null);
                const response = await axios.get(`http://backend-alb-283290471.us-east-2.elb.amazonaws.com:8090/api/fichas/paciente/${idPaciente}`);
    ...
    const consultarIA = async () => {
        ...
        try {
            ...
            const response = await axios.post(`http://backend-alb-283290471.us-east-2.elb.amazonaws.com:8090${endpoint}`, payload);
        ...
        return (
            <div className="patient-card">
                <div className="patient-header">
                    {ficha.paciente?.foto ? (
                        <img
                            src={`http://backend-alb-283290471.us-east-2.elb.amazonaws.com:8090/api/registro/${ficha.paciente.foto}`}
                            alt="Foto del paciente"
                            className="patient-photo"
                            onError={(e) => {
                                e.target.onerror = null;
                                e.target.src = `http://backend-alb-283290471.us-east-2.elb.amazonaws.com:8090/uploads/${ficha.paciente.foto}`;
                                e.target.onerror = () => {
