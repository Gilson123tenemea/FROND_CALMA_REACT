...
const TrabajosAceptados = () => {
  ...
  const cargarTrabajosAceptados = async () => {
    try {
      setLoading(true);
      const respuesta = await fetch(`http://backend-alb-283290471.us-east-2.elb.amazonaws.com:8090/api/calificaciones/trabajos-aceptados/${userId}`);
      if (respuesta.ok) {
    ...
    for (const trabajo of trabajos) {
      try {
        const respuesta = await fetch(
          `http://backend-alb-283290471.us-east-2.elb.amazonaws.com:8090/api/calificaciones/existe/${trabajo.postulacion.id_postulacion}/${userId}`
        );
        if (respuesta.ok) {
