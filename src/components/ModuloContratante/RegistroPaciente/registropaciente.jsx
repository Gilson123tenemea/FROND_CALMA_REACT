...
const RegistroPaciente = () => {
  ...
  useEffect(() => {
    if (idPaciente) {
      axios.get(`http://3.129.59.126:8090/api/registro/paciente/detalle/${idPaciente}`)
        .then(async (res) => {
          ...
          if (res.data.success) {
            ...
            setFormulario({
              ...
              contrasena: '',
              confirmarContrasena: '',
              foto: p.foto ? `http://3.129.59.126:8090/api/registro/${p.foto}` : ''
            });
  ...
  useEffect(() => {
    const cargarCedulasRegistradas = async () => {
      try {
        const res = await axios.get('http://3.129.59.126:8090/api/registro/paciente/cedulas');
  ...
  const handleActualizar = () => {
    axios.put(`http://3.129.59.126:8090/api/registro/paciente/${idPaciente}`, formData)
      .then(res => {
        if (res.data.success) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (idPaciente) {
        // ACTUALIZAR paciente
        const res = await axios.put(`http://3.129.59.126:8090/api/registro/paciente/${idPaciente}`, payload);
        if (res.data.success) {
