...
const VisualizarPacientes = () => {
  ...
  useEffect(() => {
    if (idContratante) {
      axios.get(`http://backend-alb-283290471.us-east-2.elb.amazonaws.com:8090/api/registro/paciente/contratante/${idContratante}`)
        .then(res => {
          if (res.data.success) {
          ...
          {pacientes.map((paciente, index) => (
            <div key={index} className={styles["paciente-card-vistavisu"]}>
              ...
              <div className={styles["paciente-foto-container"]}>
                <img
                  src={`http://backend-alb-283290471.us-east-2.elb.amazonaws.com:8090/api/registro/${paciente.foto}`}
                  alt="foto"
