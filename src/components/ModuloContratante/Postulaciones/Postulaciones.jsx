...
const Postulaciones = () => {
  ...
  const verificarCalificacion = async (idPostulacion, idContratante) => {
    try {
      const response = await axios.get(
        `http://softwave.online:8090/api/calificaciones/existe/${idPostulacion}/${idContratante}`
      );
  ...
  const actualizarEstado = async (idPostulacion, idPostulacionEmpleo, nuevoEstado, idAspirante, aspiranteName, jobTitle) => {
    ...
    try {
      ...
      await axios.put(
        `http://softwave.online:8090/api/postulacion/actualizar/${idPostulacion}/${userId}/${idAspirante}`,
        {
          estado: nuevoEstado,
          postulacion_empleo: {
