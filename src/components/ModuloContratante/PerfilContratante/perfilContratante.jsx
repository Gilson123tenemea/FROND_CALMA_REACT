...
const PerfilContratante = () => {
  ...
  useEffect(() => {
    const cargarDatosContratanteYEmpresa = async () => {
      ...
      try {
        const resContratante = await fetch(`http://3.129.59.126:8090/api/registro/contratante/detalle/${idContratante}`);
        ...
        if (jsonContratante.success) {
          ...
          if (data.foto) {
            setFoto(`http://3.129.59.126:8090/api/registro/${data.foto}`);
          } else {
            setFoto(null);
          }
        } else {
        ...
        const resEmpresa = await fetch(`http://3.129.59.126:8090/api/registro/empresa/contratante/${idContratante}`);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await fetch(`http://3.129.59.126:8090/api/registro/contratante/${idContratante}`, {
        method: 'PUT',
        headers: {
