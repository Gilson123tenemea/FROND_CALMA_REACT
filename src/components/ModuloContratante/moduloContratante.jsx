...
const ModuloContratante = () => {
  ...
  const cargarDatosUsuarioActual = async (contratistaId) => {
    try {
      ...
      const response = await axios.get(`http://3.129.59.126:8090/api/usuarios/${contratistaId}`);
  ...
  useEffect(() => {
    const fetchNoLeidas = async () => {
      ...
      try {
        const endpoint = `http://3.129.59.126:8090/api/notificaciones/contratante/noleidas/${contratanteId}`;
  ...
  const handleAbrirPanelUsuarios = async () => {
    ...
    try {
      ...
      const response = await axios.get(`http://3.129.59.126:8090/api/postulacion/contratista/${contratanteId}/aspirantes-para-chat`);
